...
new_hiv_long = new_hiv_raw.melt(
    id_vars=["country", "country_code"], var_name="year", value_name="new_cases"
)
...
total_hiv_long = total_hiv_raw.melt(
    id_vars=["country", "country_code"], var_name="year", value_name="total_cases"
)
...
```{python}
# Query the data to just the `SP.POP.TOTL` indicator
population_sp_pop_totl = population.query("indicator == 'SP.POP.TOTL'")

population_sp_pop_totl
```

```{python}
# Melt the data to a long format.
population_long = population_sp_pop_totl.melt(
...
```{python}
# Your code here
# select 3 countries of your choice
...
```{python}
# plot incidence rates over time using a line grap
px.line(
    hiv_data_mini,
    x="year",
    y="incidence_rate",
    color="country",
    labels={"year": "Year", "incidence_rate": "Incidence Rate per 100,000 People"},
    title="HIV Incidence Rate Over the Years",
)
```
...
px.line(
    ...
    y="prevalence_rate",
    color="country",
    labels={"year": "Year", "prevalence_rate": "Prevalence Rate per 100,000 People"},
    title=" HIV Prevalence Rate Over the Years",
)
```
...
hiv_data_combined_pop["continent"] = coco.convert(hiv_data_combined_pop["country"], to = "continent")
...
# Extract 2011 hiv dataset

hiv_pop_data_2011 = hiv_data_combined_pop.query("year == 2011")

hiv_pop_data_2011
```


Find the countries with the highest prevalence rate in 2011 for each continent


```{python}
# Obtain the countries with hiighest prevalence
max_hiv_pop_data_2011_by_continent = hiv_pop_data_2011.loc[
    hiv_pop_data_2011.groupby("continent")["prevalence_rate"].idxmax()
]

# Filter the columns of interest
max_prev_rate_2011_by_continent = max_hiv_pop_data_2011_by_continent[
    ["country", "continent", "prevalence_rate"]
]

# Sort prevalence rate in desending order
max_prev_rate_2011_by_continent.sort_values(["prevalence_rate"], ascending = False)
```

Eswatini, Bahamas, Estonia, Thailand and Papua New Guinea in Africa, America, Europe, Asia and Oceania continents respectively, had the highest HIV prevalence rate in 2011.


```{python}
# Make a bar chart comparing the 2011 prevalence rates of these countries.
px.bar(
    max_prev_rate_2011_by_continent,
    x="country",
    y="prevalence_rate",
    title="Top HIV Prevalence Rate Country per Continent, 2011",
    labels={"country": "Country", "prevalence_rate": "Prevalence Rate"},
    color="continent",
)
```
